...
from DesignerUI import OperationCompleteDialog, TableExtractWindow
...
class UserTableExtractDialog(QtWidgets.QDialog, TableExtractWindow.Ui_Dialog):
    ...
    def __init__(self, parent=None):
        super(UserTableExtractDialog, self).__init__(parent)
        self.setupUi(self)
        self.setWindowIcon(QIcon('favicon3.ico'))
        self.setWindowTitle("TableExtracter")
    ...
    def setSavePath(self):
        uformat = "Microsoft Excel 97-2003 文件 (*.xls);;"\
    ...
    def extOperate(self):
        self.sheetName = self.sheet.toPlainText()
        self.beginPage = eval(self.beginPageEdit.toPlainText())
        self.endPage = eval(self.endPageEdit.toPlainText())
        df = PDFOperations.getTable(self.sourceFileName,
                                    self.beginPage,
                                    self.endPage)
        if self.sheetName == "":
            PDFOperations.writeExcel(df, self.savePath)
        else:
            PDFOperations.writeExcel(df, self.savePath, self.sheetName)
